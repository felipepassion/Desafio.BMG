...
using Bmg.Desafio.Proposta.Application.Aggregates.PropostaAgg.AppServices;
using Bmg.Desafio.Proposta.Infra.Data.Aggregates.PropostaAgg.Repositories;
...
namespace Bmg.Desafio.Proposta.Infra.IoC {
	using Core.Infra.IoC.Extensions;
    using Core.Api.Middlewares;
	using Infra.Data.Context;
	using Domain.Aggregates.PropostaAgg.Commands.Handlers;
    ...
    public partial class IoCFactory : IBaseIoC {
		...
		void ConfigureMediatR(IServiceCollection services)
		{
            ...
            services.AddMediatR((x) => { x.RegisterServicesFromAssembly(typeof(BasePropostaAggCommandHandler<>).GetTypeInfo().Assembly); });
		...
		void ConfigureDatabase(IServiceCollection services, IConfiguration configuration)
        {
			...
			services.AddDbContext<PropostaAggContext>(options =>
